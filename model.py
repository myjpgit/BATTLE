...
class Encoder_Decoder(nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim, num_layers, dropout, dropout_adj, Adj, act, sparse):
        ...
        self.struct_decoder = GCN(in_channels=out_dim, hidden_channels=hid_dim, out_channels=in_dim, num_layers=decoder_layers,
                                  dropout=dropout, dropout_adj=dropout_adj, Adj=Adj, sparse=sparse)
    ...
    def forward(self, x, Adj, Adj_):
        # encode
        h = self.shared_encoder(x, Adj)
        # decode feature matrix
        x_ = self.attr_decoder(x, Adj_)
        # decode adjacency matrix
        h_ = self.struct_decoder(x, Adj_)
        s_ = h_ @ h_.T
...
class GCL(nn.Module):
    def __init__(self, nlayers, in_dim, hidden_dim, emb_dim, proj_dim, dropout, dropout_adj, sparse):
        ...
        # self.attr_decoder = GCN(proj_dim, hidden_dim, in_dim, nlayers, dropout, dropout_adj, sparse)
        # self.struct_decoder = GCN(proj_dim, hidden_dim, in_dim, nlayers, dropout, dropout_adj, sparse)

    def forward(self, x, Adj_, branch=None):
        z, embedding = self.encoder(x, Adj_, branch)
        # x_= self.attr_decoder(z, Adj_)
        # h_ = self.struct_decoder(z)
        return z, embedding
