...
def evaluate_AUC(y_predict, y_ture):
    ...
    roc_auc = roc_auc_score(y_ture, y_predict)
    # print(y_ture)
    # print(y_predict)
    # roc_auc = roc_auc_score(y_true=y_ture, y_score=y_predict)
    return roc_auc
    
def evaluate_precision(y_predict, y_ture, k):
    y_predict = y_predict.to("cpu").detach().numpy()
    y_ture = y_ture.to("cpu").detach().numpy()
    larget_index = heapq.nlargest(k, range(len(y_predict)), y_predict.__getitem__)
    precision_k = np.sum(y_ture[larget_index]) / k

    N = len(y_predict)
    labels = np.array(y_ture)
    pred = np.array(y_predict)
    precision_at_k = sum(labels[pred.argpartition(N - k)[-k:]]) / k

    return precision_at_k
    
def evaluate_recall(y_predict, y_ture, k):
    y_predict = y_predict.to("cpu").detach().numpy()
    y_ture = y_ture.to("cpu").detach().numpy()
    larget_index = heapq.nlargest(k, range(len(y_predict)), y_predict.__getitem__)
    recall_k = np.sum(y_ture[larget_index]) / np.sum(y_ture)
